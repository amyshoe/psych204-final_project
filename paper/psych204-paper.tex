...
Though Nathan and Koedinger were able to identify one set of circumstances where students' performance in story problems surpassed teachers' expectations, this could not invalidate such a widespread belief among students and teachers of algebra, and rather motivated the authors to follow up on their own study by tracking students' performance on more difficult algebra problems. (\textbf{citation}) introduces the notion of single-reference and double-reference problems: the first kind, like questions (A) and (B), makes reference to the unknown value only once, whereas the second kind, like questions (C) and (D) below, refer to the unknown value twice:
